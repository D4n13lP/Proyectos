...
export default function ClientsPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      data.sort((a: Cliente, b: Cliente) => a.nombre.localeCompare(b.nombre));
      setClientes(data);
    }
  }, []);
  ...
  const saveToLocalStorage = (data: Cliente[]) => {
    ...
    localStorage.setItem('clientes_db', JSON.stringify(sortedData));
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-gray-600 text-sm">
              {displayedClientes.map((cliente) => (
                <tr key={cliente.id} className="hover:bg-gray-50 border-b border-gray-100 transition-colors">
                  <td className="p-4 font-mono text-[10px] text-gray-400">{cliente.id}</td>
                  <td className="p-4 font-medium text-gray-900">{cliente.nombre}</td>
                  <td className="p-4">{cliente.telefono1}</td>
                  <td className="p-4">{cliente.rfc || '---'}</td>
                  <td className="p-4">{cliente.email || '---'}</td>
