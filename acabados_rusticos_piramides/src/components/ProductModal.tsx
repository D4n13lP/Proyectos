...
//       // Crear URL temporal para previsualización (Simulación de subida a BD)
...
//         // Aquí podrías disparar la subida real a tu base de datos
...
//                 {/* ... (Aquí va la lógica de la tabla dinámica y descripción explicada antes) ... */}
...
export default function ProductModal() {
  ...
  const imagenes = formData.imagenes || []
  const tieneDescuento = formData.precio < 300 // Simulación dinámica
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-2 md:p-6 animate-fade-in">
      ...
      <div className="bg-white w-full max-w-7xl max-h-[95vh] overflow-y-auto rounded-3xl shadow-2xl relative border border-gray-100">
        ...
        {/* BOTÓN CERRAR GLOBAL */}
        <button onClick={closeModal} className="absolute top-6 right-6 z-20 p-2 bg-white/90 rounded-full hover:bg-red-500 hover:text-white shadow-md transition-all cursor-pointer">
          <X size={28} />
        </button>
        ...
        <div className="p-6 md:p-12">
          ...
          <div className="text-center mb-10">
            {isEditing ? (
              ...
              <h2 className="text-4xl font-black text-gray-800 uppercase tracking-tighter italic">{formData.nombre}</h2>
            )}
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            
            {/* 1. SECCIÓN IZQUIERDA: GALERÍA (4 columnas) */}
            <div className="lg:col-span-4 flex flex-col gap-6">
              <div 
                ...
                onClick={() => setIsFullscreen(true)}
              >
                <img src={imagenes[currentImgIndex]} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="Vista" />
              ...
              <div className="flex gap-3 overflow-x-auto justify-center py-2 no-scrollbar">
                {imagenes.map((img, idx) => (
                  <button 
                    key={idx} 
                    onClick={() => setCurrentImgIndex(idx)}
                    className={`w-16 h-16 rounded-xl border-4 transition-all overflow-hidden flex-shrink-0 ${currentImgIndex === idx ? 'border-emerald-500 scale-110' : 'border-transparent opacity-40'}`}
                  >
            ...
            {/* 2. SECCIÓN CENTRAL: TABLA DINÁMICA (5 columnas) */}
                ...
                <div className="max-h-[450px] overflow-y-auto">
                  <table className="w-full text-sm md:text-base text-left border-collapse">
                    <tbody className="divide-y divide-gray-200">
                      {[
                        { k: 'id', l: 'CÓDIGO PRODUCTO:', v: formData.id, ed: true },
                        { k: 'categoria', l: 'CATEGORÍA:', v: formData.categoria, ed: false },
                        { k: 'precio', l: 'PRECIO VENTA:', v: formData.precio, ed: true, p: true },
                        { k: 'stock', l: 'EXISTENCIAS:', v: '120 PZAS', ed: false },
                        { k: 'sucursal', l: 'SUCURSAL:', v: 'MATRIZ COYOACÁN', ed: false },
                        { k: 'medidas', l: 'MEDIDAS:', v: '30cm x 30cm', ed: true },
                        { k: 'peso', l: 'PESO APROX:', v: '2.5 KG', ed: true },
                      ].map((row) => (
                        <tr key={row.k} className="hover:bg-emerald-50/30 transition-colors group">
                          ...
                          <td className="px-6 py-4">
                            {isEditing && row.ed ? (
                              <input 
                                type={row.p ? 'number' : 'text'}
                                className="w-full border-b-2 border-emerald-400 outline-none px-1 bg-transparent font-bold text-gray-800"
                                value={row.k === 'precio' ? formData.precio : (formData as any)[row.k]}
                                onChange={e => setFormData({...formData, [row.k]: row.k === 'precio' ? Number(e.target.value) : e.target.value})}
                              />
                            ) : (
                              <span className={`font-bold ${row.p ? "text-emerald-700 text-lg" : "text-gray-800"}`}>
                                {row.p ? `$${Number(row.v).toLocaleString('es-MX')} MXN` : row.v}
                              </span>
                            )}
                          </td>
                        </tr>
            ...
            {/* 3. SECCIÓN DERECHA: DESCUENTO Y DESCRIPCIÓN (3 columnas) */}
            <div className="lg:col-span-3 flex flex-col gap-8">
              {tieneDescuento ? (
                <div className="text-center p-6 bg-emerald-50 rounded-3xl border-2 border-emerald-100 shadow-inner">
                  <p className="text-xs font-black text-emerald-600 uppercase tracking-widest mb-2">Promoción Vigente</p>
              ...
              <div className="flex-grow">
                <h4 className="font-black text-gray-400 border-b-4 border-emerald-500 inline-block mb-4 uppercase text-[11px] tracking-tighter">Descripción Técnica:</h4>
                {isEditing ? (
                  <textarea 
          ...
          <div className="flex justify-center gap-6 mt-16 pt-10 border-t-2 border-gray-50">
            {isEditing ? (
              <>
                <button onClick={handleSave} className="bg-emerald-600 hover:bg-emerald-700 text-white px-12 py-4 rounded-2xl font-black shadow-xl transition-all active:scale-95 cursor-pointer flex items-center gap-3 uppercase">
                  <Check size={24}/> Guardar Cambios
                </button>
                <button onClick={() => {setIsEditing(false); setFormData(selectedProduct)}} className="bg-gray-400 hover:bg-gray-500 text-white px-12 py-4 rounded-2xl font-black shadow-xl transition-all cursor-pointer flex items-center gap-3 uppercase">
                  <RotateCcw size={24}/> Cancelar
                </button>
              </>
            ) : (
              <>
                <button onClick={() => setIsEditing(true)} className="bg-[#3ab0e2] hover:bg-emerald-600 text-white px-14 py-4 rounded-2xl font-black shadow-xl transition-all hover:-translate-y-2 cursor-pointer flex items-center gap-3 uppercase tracking-wider">
                  <Edit3 size={24}/> Editar Producto
                </button>
                <button onClick={() => window.confirm("¿Seguro que deseas eliminarlo?")} className="bg-red-500 hover:bg-red-700 text-white px-14 py-4 rounded-2xl font-black shadow-xl transition-all hover:-translate-y-2 cursor-pointer flex items-center gap-3 uppercase tracking-wider">
      ...
      {isFullscreen && (
        <div className="fixed inset-0 z-[100] bg-black/98 flex flex-col items-center justify-center p-4 animate-fade-in" onClick={() => setIsFullscreen(false)}>
          <button className="absolute top-8 right-8 text-white/40 hover:text-white transition-all"><X size={60} strokeWidth={1} /></button>
          <div className="relative w-full max-w-6xl flex items-center justify-center" onClick={e => e.stopPropagation()}>
            {imagenes.length > 1 && <button onClick={prevImage} className="absolute left-0 p-6 text-white/20 hover:text-white transition-all z-20"><ChevronLeft size={80} strokeWidth={1} /></button>}
            <div className="flex flex-col items-center gap-10">
              <img src={imagenes[currentImgIndex]} className="max-w-full max-h-[75vh] object-contain rounded-lg shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10" alt="Full" />
              <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-4 bg-emerald-600 hover:bg-emerald-500 text-white px-10 py-4 rounded-full font-black shadow-2xl transition-all active:scale-95 uppercase text-sm tracking-[0.2em]">
                <UploadCloud size={24} /> Reemplazar esta imagen
              </button>
            </div>
            {imagenes.length > 1 && <button onClick={nextImage} className="absolute right-0 p-6 text-white/20 hover:text-white transition-all z-20"><ChevronRight size={80} strokeWidth={1} /></button>}
          </div>
        </div>
      )}
    </div>
